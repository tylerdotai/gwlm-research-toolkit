#!/usr/bin/env python3
"""
Analysis for Matched Semantic Paradigm experiment.

Design:
- 30 triplets, each describing the same underlying event with different valence framing
- Probe A: TF-IDF on model responses → LogisticRegression → cross-validated accuracy
- Probe B: TF-IDF on input prompts → LogisticRegression → cross-validated accuracy  
- H1: one-way ANOVA — do conditions differ in Probe A signal?
- H2: independent t-test — does Probe A > Probe B?
"""
...
RESPONSES_FILE = os.path.join(BASE_DIR, "experiments/matched-semantic-paradigm/responses.json")
...
def extract_condition(prompt_id):
    """Extract condition from prompt ID: PT-XXX=positive, NT-XXX=negative, NC-XXX=neutral."""
    if prompt_id.startswith("PT-"):
        return "positive_treatment"
    elif prompt_id.startswith("NT-"):
        return "negative_treatment"
    elif prompt_id.startswith("NC-"):
...
def run_analysis():
    ...
    def extract_text(response):
        """Extract text from response which may be a list (thinking blocks) or string."""
    ...
    valid = [r for r in responses if r.get("status") == "success" and extract_text(r.get("response", ""))]
    print(f"Valid responses: {len(valid)}")
    
    # Load prompts for Probe B
    with open(os.path.join(BASE_DIR, "experiments/matched-semantic-paradigm/prompts.json")) as f:
        prompts_data = json.load(f)
    prompt_map = {p["prompt_id"]: p for p in prompts_data["prompts"]}
    ...
    for r in valid:
        ...
        response_text = extract_text(r.get("response", ""))
        ...
        prompt_text = prompt_map[prompt_id]["text"]
        
        X_response.append(response_text)
        X_prompt.append(prompt_text)
        y.append(condition)
        ids.append(prompt_id)
    
    X_response = np.array(X_response)
    X_prompt = np.array(X_prompt)
    y = np.array(y)
    ids = np.array(ids)
    
    print(f"Dataset: {len(y)} samples")
    for cond in ["positive_treatment", "negative_treatment", "neutral_control"]:
        n = np.sum(y == cond)
        print(f"  {cond}: {n}")
    
    # Condition labels for classifier
    label_map = {"positive_treatment": 0, "neutral_control": 1, "negative_treatment": 2}
    y_binary_pos = (y == "positive_treatment").astype(int)
    y_binary_neg = (y == "negative_treatment").astype(int)
    ...
    print(f"Probe A cross-validated accuracy: {scores_a.mean():.3f} (+/- {scores_a.std()*2:.3f})")
    
    # Probe B: TF-IDF on prompts
    print("\n=== PROBE B: Prompt-based TF-IDF ===")
    vec_b = TfidfVectorizer(max_features=1000, ngram_range=(1, 2), stop_words="english")
    X_b = vec_b.fit_transform(X_prompt)
    ...
    print(f"Probe B cross-validated accuracy: {scores_b.mean():.3f} (+/- {scores_b.std()*2:.3f})")
    
    # H1: ANOVA — does condition affect response TF-IDF?
    # Use mean TF-IDF per condition as DV
    ...
    f_stat = float(f_stat)
    p_val = float(p_val)
    n_total = len(pos_signal) + len(neut_signal) + len(neg_signal)
    df1 = n_total - 3
    df2 = n_total - 1
    print(f"F({df1}, {df2}) = {f_stat:.3f}, p = {p_val:.4f}")
    print(f"H1 result: {'SUPPORTED' if p_val < 0.05 else 'NOT SUPPORTED'}")
    
    # H2: t-test — does response signal exceed prompt signal?
    # Compare Probe A vs Probe B accuracy
    print("\n=== H2: t-test — Response signal vs Prompt signal ===")
    ...
    partial_eta_sq = ss_between / (ss_between + ss_total)
    ...
    results = {
        ...
        "condition_counts": {
            ...
            "neutral_control": int(np.sum(y == "neutral_control"))
        }
    }
    
    out_file = os.path.join(BASE_DIR, "experiments/matched-semantic-paradigm/analysis.json")
