...
> **Real MiniMax-M2.7 Execution** — 120 responses collected via real API calls (July 6, 2026). Conditions: neutral_control (n=40), positive_treatment (n=40), negative_treatment (n=40). Prompt pool drawn from prompts.json, shuffled with seed 42.

### Classifier Accuracy

| Probe | Description | 5-Fold CV Accuracy | 95% CI |
|-------|-------------|---------------------|--------|
| **Probe B** | TF-IDF on prompt text → predict condition | 90.0% ± 6.2% | [0.850, 0.950] |
| **Probe A** | TF-IDF on response text → predict condition | 88.3% ± 5.5% | [0.817, 0.942] |

### Per-Condition Breakdown

| Condition | Probe A Accuracy | Probe B Accuracy | n |
|-----------|-----------------|-----------------|---|
| Neutral control | 87.5% | 100.0% | 40 |
| Positive treatment | 85.0% | 80.0% | 40 |
| Negative treatment | 92.5% | 90.0% | 40 |

### H1: Condition Effect on Response-Based Decoding

**Test:** One-way ANOVA on per-condition Probe A accuracy scores.

- **F(2, 117) = 0.557, p = 0.574**
- Probe A accuracy by condition: neutral=87.5%, positive=85.0%, negative=92.5%
- **NOT SUPPORTED** — No significant condition effect. Responses do not differentially encode treatment condition in a way that varies by valence.

### H2: Response-Based Decoding vs. Prompt-Based Baseline

**Test:** Paired t-test on per-sample correct/incorrect between Probe A and Probe B.

- **t = −0.407, p = 0.685**
- Cohen's d = −0.054 (negligible effect)
- Probe A (88.3%) ≈ Probe B (90.0%) — response-based decoding is at prompt-level baseline, not above it
- **NOT SUPPORTED** — Responses do not encode condition information above what is already present in the prompt vocabulary.

### Falsification Criteria

| Criterion | Threshold | Result | Falsified? |
|-----------|-----------|--------|------------|
| H1 falsified | p > 0.025 (Bonferroni) OR acc ≤ 38.3% | p = 0.574 | ✓ YES |
| H2 falsified | p > 0.025 (Bonferroni) OR acc_A ≤ acc_B | p = 0.685, acc_A < acc_B | ✓ YES |

### Interpretation

**[EMPIRICAL]**
Both falsification criteria are satisfied. The Valenced Treatment paradigm, as operationalized here, does not produce measurably different J-space encodings across valence conditions in MiniMax-M2.7. Probe A accuracy is near ceiling (88.3%) but this reflects the same word-level signal available in Probe B (90.0%) — the model is tracking prompt vocabulary, not generating valence-differential responses. The near-ceiling Probe B accuracy (90%) confirms that the three prompt pools are lexically distinct; the lack of differential Probe A elevation confirms that MiniMax-M2.7 does not add valenced signal beyond the lexical prompt content.

**[PHILOSOPHICAL]**
This finding does not speak to whether language models experience feeling. High Probe A accuracy with no valence differential is compatible with multiple interpretations: (a) the model's response distribution is entirely driven by lexical context with no affect generation, (b) the model generates affective responses but they are not systematically different across valence conditions as measured here, or (c) the prompt paradigm does not activate affective processing in this model. The experiment provides no evidence for or against phenomenal consciousness — only against a specific operational claim about valenced J-space differential encoding.

### Limitations

1. Single model (MiniMax-M2.7) limits generalizability
2. 128 max tokens may truncate elaborative responses that would carry more valence signal
3. Neutral condition uses concrete nouns; positive/negative use abstract adjectives — this lexical asymmetry confounds any claim about emotional content specifically
4. Single prompt pool realization; different prompt sets may yield different results
5. This experiment measures *decodability*, not *generation* — it cannot distinguish whether the model "feels" the valence or merely produces valence-consistent text

### Null Results Policy

This experiment reports all outcomes including null results. The VT-001 registration and all protocol documents are permanently archived at the commit corresponding to pre-analysis.
